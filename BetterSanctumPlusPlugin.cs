...
public class BetterSanctumPlusPlugin : BaseSettingsPlugin<BetterSanctumPlusSettings>
{
    ...
    private void ExportTrackingWorkbook()
    {
        ...
        if (written < 0)
        {
            LogError($"[BetterSanctumPlus] could not export the workbook: {error}", 30);
            return;
        }
        ...
        {
            LogError($"[BetterSanctumPlus] exported {written} rows, but {error}", 30);
            return;
        }

        LogMessage($"[BetterSanctumPlus] exported {written} rows to sanctum-run-wide.xlsx", 30);
    }
    ...
    private void ExportTrackingTemplate()
    {
        ...
        if (written < 0)
        {
            LogError($"[BetterSanctumPlus] could not write the template: {error}", 30);
            return;
        }

        LogMessage($"[BetterSanctumPlus] wrote a blank template for {TemplateRuns} runs to sanctum-run-template.xlsx", 30);
    }
